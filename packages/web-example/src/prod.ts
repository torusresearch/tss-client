...
const runTest = async (testConfig: TestConfig) => {
  const tssImportUrl = "https://sapphire-1.auth.network/tss/v1/clientWasm";
  ...
  await client.ready().catch((err) => {
    throw new Error(`tss-error during compute: ${err}`);
  });
  console.log("ready for signing");

  // initiate signature.
  const signature = await client.sign(tss, msgHash.toString("base64"), true, msg, "keccak256", { signatures }).catch((err) => {
    throw new Error(`${testConfig.label} error during sign: ${err}`);
  });
  ...
  await client.cleanup(tss, { signatures }).catch((err) => {
    throw new Error(`${testConfig.label} error during cleanup: ${err}`);
  });
  sockets.map((soc) => {
...
const tssSigningTest = async (testConfig: TestConfig) => {
  try {
