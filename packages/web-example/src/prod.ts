...
import { Client, DELIMITERS, generateEndpoints, getDKLSCoeff, getTSSPubKey, setupSockets } from "@toruslabs/tss-client";
...
const runTest = async (testConfig: TestConfigType) => {
  ...
  const { signatures, postboxkey, nodeIndexes } = await fetchPostboxKeyAndSigs(
    { verifierName: verifier, verifierId: torusSp.verifierId },
    network
  ).catch((err) => {
  ...
  const [sockets] = await Promise.all([setupSockets(tssWSEndpoints, session.split(DELIMITERS.Delimiter4)[1])]);
