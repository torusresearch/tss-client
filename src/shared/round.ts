...
export default class Rounds {
  private _roundTrackerLocks: Record<string, Promise<unknown>> = {};
  ...
  roundRunner = async ({
    ...
    try {
      ...
      // acquire lock on tag
      if (this._roundTrackerLocks[tag] === undefined) {
        this._roundTrackerLocks[tag] = new Promise<void>((res) => {
          this._release = () => {
            delete this._roundTrackerLocks[tag]; // release lock
            res();
          };
        });
      } else {
        // wait to acquire lock
        await this._roundTrackerLocks[tag].then();
  ...
  private async _executeRoundTracker({ nodeKey, db, tag, roundName, party, serverSend, serverBroadcast, wsNotify, clientReadyResolve, tss }) {
    ...
    if (roundName === "round_1_commitment_broadcast") {
      await this._executeRoundOne({ roundTracker, db, tag, tss, nodeKey, serverBroadcast, index, parties, endpoints });
    } else if (roundName === "round_1_commitment_received") {
      await this._executeRoundOneCommitmentReceived({ roundTracker, db, tag, nodeKey, serverSend, party, index, parties, h1h2Ntildes, endpoints });
    } else if (roundName === "round_2_MessageA_received") {
      await this._executeRoundTwoMessageAReceived({
        roundTracker,
        party,
        roundName,
        db,
      ...
      await this._executeRoundTwoMessageBs({ roundTracker, roundName, party, parties, index, tag, db, endpoints, nodeKey, gwis, serverBroadcast });
    } else if (roundName === "round_3_Delta_received") {
      await this._executeRound3DeltaReceived({ roundTracker, party, db, tag, parties, nodeKey, serverBroadcast, endpoints, index, tss });
    } else if (roundName === "round_4_Di_received") {
      await this._executeRound4DiReceived({
        roundTracker,
        party,
        db,
        tag,
      ...
      await this._executeRoundFive({ roundTracker, parties, party, db, tag, index, nodeKey, serverBroadcast, roundName, tss, endpoints, eks });
    } else if (roundName === "round_6_Rsigmai_received") {
      await this._executeRoundSix({ roundTracker, parties, party, clientReadyResolve, wsNotify, index, tag, db, tss, pubkey });
    } else {
      throw new Error(`roundName ${roundName} not found`);
    }
  }

  private async _executeRoundOne({ roundTracker, db, tag, tss, nodeKey, serverBroadcast, index, parties, endpoints }) {
  ...
  private async _executeRoundOneCommitmentReceived({ roundTracker, party, parties, index, db, tag, nodeKey, endpoints, h1h2Ntildes, serverSend }) {
    if (party === undefined) {
  ...
  private async _executeRoundTwoMessageAReceived({
    roundTracker,
    party,
    roundName,
    db,
  ...
  private async _executeRoundTwoMessageBs({ roundTracker, roundName, party, parties, index, tag, db, endpoints, nodeKey, gwis, serverBroadcast }) {
  ...
  private async _executeRound3DeltaReceived({ roundTracker, party, db, tag, parties, nodeKey, serverBroadcast, endpoints, index, tss }) {
  ...
  private async _executeRound4DiReceived({
    roundTracker,
    party,
    parties,
    db,
    ...
    if (this._allTrue(roundTracker.round_4_Di_received)) {
      ...
      for (const i in roundTracker.round_5_proof_pdl_sent) {
        roundTracker.round_5_proof_pdl_sent[i] = true;
      }
      await db.set(`tag-${tag}:rounds`, JSON.stringify(roundTracker));
      this._release();
  ...
  private async _executeRoundFive({ roundTracker, parties, party, db, tag, index, nodeKey, serverBroadcast, roundName, tss, eks, endpoints }) {
  ...
  private async _executeRoundSix({ roundTracker, parties, party, clientReadyResolve, wsNotify, index, tag, db, tss, pubkey }) {
