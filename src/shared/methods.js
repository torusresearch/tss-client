...
const methods = (rounds) => {
  return {
    generateNodeInfo: async (db, nodeKey, index) => {
      const [keys, ek, h1h2Ntilde] = tss.generate_keys(parseInt(index));
    ...
    setTagInfo: async (db, nodeKey, tag, pubkey, endpoints, parties, gwis, eks, h1h2Ntildes) => {
      ...
      await db.set(`tag-${tag}:rounds`, JSON.stringify(rounds.createRoundTracker(parties, index)));
    },
    tssSign: async (db, nodeKey, tag, msg_hash) => {
      const { pubkey } = await rounds.getTagInfo(db, tag);
